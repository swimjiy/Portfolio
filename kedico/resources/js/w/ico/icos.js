...
$(document).ready(function(){
	$("ul.gnb_list li").eq(1).addClass("active");
	...
	if(ico_status){
		$("div.search_wrap li.category_item").eq(3).find("a").each(function(){
	...
	$("div.search_wrap div.search_box_xl button.btn_search").on("click", function(){
		setSearchData();
		searchICO();
	});
	...
	// 상세 검색 버튼 클릭 시 이벤트 발생
	$("div.search_wrap button.btn_round").on("click", function(){
		if($(this).hasClass("active")){
			// 상세 검색 종료
			$(this).removeClass("active");
			$("div.search_wrap div.search_category").slideUp();
		}else{
			// 상세 검색 실행
			$(this).addClass("active");
			$("div.search_wrap div.search_category").slideDown();
		}
	});
	
	// 상세 검색에서 X 버튼 클릭 시 이벤트 발생 - 상세 검색 종료
	$("div.search_wrap button.btn_close").on("click", function(){
		$("div.search_wrap button.btn_round").removeClass("active");
		$("div.search_wrap div.search_category").slideUp();
	});
	
	// 상세 검색 아이템 항목 클릭 시 이벤트 발생(상태 제외)
	$("div.ico_search a.btn_square").on("click", function(){
		if($(this).hasClass("active")){
			$(this).removeClass("active");
		}else{
			$(this).addClass("active");
	...
	// 상세 검색 - 상태 아이템 클릭 시 이벤트 발생
	$("div.ico_status a.btn_square").on("click", function(){
		$("div.ico_status a.btn_square").removeClass("active");
		$(this).addClass("active");
...
$(document).on("click", "div.record_box a.btn_record", function(){
	...
	if(class_name == 'btn_ico_status'){ // 상태 삭제
		$("div.search_wrap li.category_item").eq(3).find("a").removeClass("active");
	}
...
function setSearchData(){
	...
	// 조건 - 내가 평가한 ICO 제외
	if($("div.search_wrap li.category_item").eq(5).find("a.except_me").hasClass("active")){
